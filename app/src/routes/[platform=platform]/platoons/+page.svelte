...
<div
	class="min-h-screen bg-slate-800 bg-fixed bg-cover bg-center bg-no-repeat text-white"
	style="background-image: url('/bg/british-squad.webp')"
>
		...
		<div class="grid min-h-screen grid-cols-1 sm:grid-cols-2 2xl:grid-cols-3">
			<div class="to-black-0 h-full bg-gradient-to-r from-black/80 p-8 md:p-16 lg:p-24">
				<div class="mx-auto">
					<div>
						<Back />
						<h1 class="mb-12 mt-2 text-4xl font-bold">{platformName} Platoons</h1>
					</div>
					<div class="flex flex-col gap-1">
						{#each data.platoons as platoon}
							<a href={`./${platoon.id}/`}>
								<p
