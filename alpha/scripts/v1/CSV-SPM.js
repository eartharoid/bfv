...
console.log('Calculating scorePerMinute for %d games', recent.length);

writeFileSync(
	`generated/${PLATFORM}-${USERNAME}-spm.csv`,
	stringify([
		['Timestamp', 'scorePerMinute'],
		...recent
			.map(game => {
				...
				const stat = player.stats.find(s => s.metadata.key === 'scorePerMinute').value;
