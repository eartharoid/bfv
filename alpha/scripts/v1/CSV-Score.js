...
console.log('Calculating sc_roundscore for %d games', recent.length);

writeFileSync(
	`generated/${PLATFORM}-${USERNAME}-score.csv`,
	stringify([
		['Timestamp', 'sc_roundscore'],
		...recent
			.map(game => {
				...
				const stat = player.stats.find(s => s.metadata.key === 'sc_roundscore').value;
